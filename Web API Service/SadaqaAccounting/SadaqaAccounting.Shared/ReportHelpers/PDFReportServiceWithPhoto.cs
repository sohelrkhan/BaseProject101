...
{
    public class PDFReportServiceWithPhoto<T, F> where F : IFilterItem
    {
        public PDFReportServiceWithPhoto() => QuestPDF.Settings.License = LicenseType.Community;
                ...
                container.Page(page =>
                {
                    ...
                    page.Content().Element(content =>
                    {
                        content.Column(column =>
                        {
                            ...
                            column.Item().Padding(3).Element(ComposeTable(dataList));
                        });
                    });
        ...
        private static Action<IContainer> ComposeTable(ICollection<T> dataList) => container =>
        {
            ...
            container.PaddingTop(5).PaddingTop(5).Table(table =>
            {
                ...
                static IContainer CellStyle(IContainer container) =>
                    container.Border(1).BorderColor(Colors.Grey.Lighten1).Padding(5).AlignMiddle().AlignCenter();

                static IContainer CellStyleHeader(IContainer container) =>
                    container.Border(1).BorderColor(Colors.Grey.Lighten1).Background(Colors.Grey.Lighten2).Padding(3).AlignMiddle().AlignCenter();
            });
        };
    }
}