...
@Component({
  selector: "app-role-feature-action-map",
  templateUrl: "./role-feature-action-map.component.html",
  styleUrls: ["./role-feature-action-map.component.css"],
  standalone: true,
  imports: [
    ReactiveFormsModule,
    FormsModule,
...
export class RoleFeatureActionMapComponent implements OnInit {
  ...
  roleActionMappingCreateModel: RoleActionMappingCreateModel = new RoleActionMappingCreateModel();
  ...
  onClickAccessMapping(): void {
    this.roleActionMappingCreateModel.featureActionMappingCreateModel =
      this.featureActionsMappingsCreate;
    ...
    if (isValidUserAccessCreateFrom) {
      this.spinnerService.show();
      this.roleMappingService.create(this.roleActionMappingCreateModel).subscribe(
        (result: RoleActionMappingCreateModel) => {
          this.spinnerService.hide();
          this.toastrService.success("Role Action Mapped successfully.", "Success");
          this.users = [];
          this.actions = [];
          this.featureActionsMappings = [];
          this.getSelectList();
          this.roleActionMappingCreateModel.roleName = null;
          this.roleTomSelect.setValue(null);
  ...
  private getUserAccessFromValidResult(): boolean {
    if (this.roleActionMappingCreateModel.featureActionMappingCreateModel.length == 0) {
