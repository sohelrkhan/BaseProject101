...
import { ExpenseGridModel, ExpenseService, PaginatedResponseOfExpenseGridModel, PaginationRequest, UserModel } from '../../../../../../api/base-api';
...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, NgxSpinnerModule, CommonModule, HttpClientModule, OrdinalDatePipe],
...
export class ExpensesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  expensesId: number | null = null;

  constructor(private expenseService: ExpenseService, private spinnerService: NgxSpinnerService, private toastrService: CustomTosterServiceService,
  ...
  private setInitialPagination(): void {
    ...
    this.pagination.sortField = "expenseCategoryName";
  ...
  private getExpenses(): void {
    ...
    this.expenseService.getAll(this.pagination).subscribe((result: PaginatedResponseOfExpenseGridModel) => {
    ...
    (error: any) => {
      this.spinnerService.hide();
      this.loading = false;
      this.toastrService.error("Expenses cannot load at this time! Please, try again.", "Error");
    });
  }

  openDeleteModal(selectionId: number): void {
    this.expensesId = selectionId;
  }
  ...
  onClickDeleteExpense(): void {
    ...
    this.expenseService.delete(this.expensesId).subscribe((result: boolean) => {
      this.spinnerService.hide();
      this.toastrService.success("Expense deleted.", "Successful.");
      this.getExpenses();
    },
    (error: any) => {
      this.spinnerService.hide();
      this.toastrService.error("Expense cannot be deleted! Please try again.");
      return;
    });
  }
